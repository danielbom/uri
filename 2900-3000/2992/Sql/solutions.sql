...
resultado AS (
  SELECT d.cod_dep, MAX(sm.valor) AS valor
  FROM salario_medio_divisao sm
  JOIN divisao d ON d.cod_divisao = sm.cod_divisao
  GROUP BY d.cod_dep
)
SELECT dp.nome as departamento
    , dv.nome as divisao
    , ROUND(sm.valor, 2) as media
FROM resultado r
JOIN departamento dp ON dp.cod_dep = r.cod_dep
JOIN divisao dv ON dv.cod_dep = r.cod_dep
JOIN salario_medio_divisao sm ON sm.cod_divisao = dv.cod_divisao
WHERE r.valor = sm.valor
ORDER BY media DESC;
