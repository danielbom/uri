...
-- Chat GPT proposal: change LEFT JOIN with NOT EXISTS
...
WHERE l.available AND r.available
  AND NOT EXISTS (
    SELECT 1 FROM chairs ll
    WHERE ll.queue = l.queue AND ll.id = l.id - 1 AND ll.available
  )
  AND NOT EXISTS (
    SELECT 1 FROM chairs rr
    WHERE rr.queue = r.queue AND rr.id = r.id + 1 AND rr.available
  )
ORDER BY l.id;
